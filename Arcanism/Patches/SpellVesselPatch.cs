...
{
    /* Just a means of hijacking Control Chant ending logic. */
    ...
    class SpellVessel_CompleteEarly
    {
    ...
    /* If Twin Spell was used, EndSpell is cancelled, target is changed, and the spell is resolved again on the new target -- until no twinned targets are left. */
    ...
    class SpellVessel_CalcDmgBonus
    {
        ...
        public static int CalculateSpellDamage(CastSpell source, int baseDamage, bool resonating)
        {
            ...
            if (resonating)
            {
                var roaringEchoesRank = source.MyChar.MySkills.GetAscensionRank(ROARING_ECHOES_ASCENSION_ID);
                int res = source.MyChar.MyStats.GetCurrentRes();
            ...
            int intelligence = source.MyChar.MyStats.GetCurrentInt();
            int proficiency = source.MyChar.MyStats.IntScaleMod;
            int level = source.MyChar.MyStats.Level;
